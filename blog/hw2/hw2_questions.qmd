...
For independent observations Y₁, Y₂, ..., Yₙ ~ Poisson(λ),
the likelihood function is:

L(λ | Y) = ∏[i=1 to n] [e^(−λ) * λ^(Yᵢ) / Yᵢ!]
         = e^(−nλ) * λ^(∑Yᵢ) / ∏Yᵢ!
...
To find the MLE for λ in a Poisson distribution:

1. Take the derivative of the log-likelihood with respect to λ:
